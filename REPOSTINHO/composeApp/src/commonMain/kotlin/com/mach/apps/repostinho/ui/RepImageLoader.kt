 ...
 *
 * Existe por um motivo só: a foto do morador é servida pelo `banco-api`, atrás da mesma
 * credencial do resto. O Coil, sozinho, faria um GET sem cabeçalho nenhum e toda foto
 * voltaria 401 — com o monograma no lugar, que é indistinguível de "morador sem foto".
 *
 * O token só é anexado em chamadas para o próprio banco-api: uma foto que aponte para
 * fora não leva a credencial do morador junto.
 */
fun repImageLoader(context: PlatformContext, tokens: AuthTokenProvider): ImageLoader =
    ImageLoader.Builder(context)
    .components {
        add(KtorNetworkFetcherFactory(httpClient = { authorizedClient(tokens) }))
    }
 ...
 */
private fun repToken(tokens: AuthTokenProvider) = createClientPlugin("RepToken") {
    onRequest { request, _ ->
        if (apiHost.isNotBlank() && request.url.host == apiHost) {
            tokens.idToken()?.let { request.header("Authorization", "Bearer $it") }
        }
    }
}

private fun authorizedClient(tokens: AuthTokenProvider): HttpClient = HttpClient {
    install(repToken(tokens))
}
